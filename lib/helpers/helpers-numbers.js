...
var helpers = {
  ...
  */
  formatPhoneNumber: function (num) {
    var doesStartWithNumberOne = false;
    num = num.toString();

    if( num.substring( 0, 1 ) == 1 ) {
      doesStartWithNumberOne = true;
      num = num.slice( 1 );
    }

    return ( ( doesStartWithNumberOne ) ? "1 " : "" ) + "(" + num.substr(0, 3) + ") " + num.substr(3, 3) + "-" + num.substr(6, 4);
  },

 /**
  * {{formatPhoneNumberDashes number}}
  * Output a formatted phone number
  * @author: http://bit.ly/QlPmPr
  * @param  {Number} phoneNumber [8005551212]
  * @return {Number}             [800-555-1212]
  */
  formatPhoneNumberDashes: function (num) {
    var doesStartWithNumberOne = false;
    num = num.toString();
